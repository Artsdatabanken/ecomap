...
export default class MapLayerStack extends React.Component {
  ...
  render () {
    ...
    for (const id in this.props.layers) {
      ...
      r.push(<PointBasedLayer key={id} id={id} layer={layer}
...
const PointBasedLayer = ({id, layer, viewport, onUpdate}) =>
  <ArtskartDataSourceContainer dataUrl={layer.dataUrl}>
    <EcoMapLayer key={id} id={id} layer={layer}
      viewport={viewport}
      onUpdate={onUpdate}
      scalingFactor={128 * 5000 * layer.paint.radius} zoomFactor={2} />
  </ArtskartDataSourceContainer>

const EcoMapLayer = ({ id, layer, data, viewport, scalingFactor, zoomFactor, onUpdate }) => {
...
//      return <Heatmap title={layer.title} dataUrl={layer.dataUrl} viewport={viewport} />
    case 'scatterplot':
      return <Scatterplot title={layer.title} data={data} viewport={viewport}
        {...paint} onUpdate={onUpdate}
/>
    case 'hexagon':
      return <Hexagon title={layer.title} data={data} viewport={viewport}
        {...paint} onUpdate={onUpdate}
       />
    default:
